...
for (const card of FLASHCARDS) {
  if (!card.id || !/^fc-m(?:[1-9]|1[0-2])-/.test(card.id)) fail(`invalid seed ID: ${card.id || '(missing)'}`);
  if (!Number.isInteger(card.module) || card.module < 1 || card.module > 12) fail(`invalid module on ${card.id}`);
...
console.log(`Flashcard QA passed: ${FLASHCARDS.length} cards across M1–M12, unique IDs/terms and source-specific provenance.`);
