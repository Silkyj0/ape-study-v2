...
function learningProgress(prior, fallback = {}) {
  return {
    ...
    recentConfidenceLevels: prior?.recentConfidenceLevels || [],
    lowConfidenceCount: prior?.lowConfidenceCount || 0,
    preAnswerConfidenceCounts: {
      confident: prior?.preAnswerConfidenceCounts?.confident || 0,
      unsure: prior?.preAnswerConfidenceCounts?.unsure || 0,
      guessing: prior?.preAnswerConfidenceCounts?.guessing || 0,
    },
    confidentWrongCount: prior?.confidentWrongCount || 0,
    lastResult: prior?.lastResult ?? null,
    lastConfidence: prior?.lastConfidence ?? null,
    lastConfidenceAt: prior?.lastConfidenceAt || null,
    lastPreAnswerConfidence: prior?.lastPreAnswerConfidence ?? null,
    lastPreAnswerConfidenceAt: prior?.lastPreAnswerConfidenceAt || null,
